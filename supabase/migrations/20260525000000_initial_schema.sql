-- Happus Tadka — initial schema (custom staff auth via Next.js API + service role)

create extension if not exists "pgcrypto";

-- Default restaurant (single-tenant for now)
...
create index idx_transactions_restaurant_date on public.transactions (restaurant_id, tx_date desc);
create index idx_ledger_entries_restaurant_date on public.ledger_entries (restaurant_id, entry_date desc);
create index idx_kitchen_orders_restaurant_status on public.kitchen_orders (restaurant_id, status);

-- Seed default restaurant + admin (password: admin123)
...
-- RLS: enabled; no policies for anon/authenticated — app uses service role via Next.js API only
